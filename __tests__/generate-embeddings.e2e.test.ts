...
describe('Embedding Generation E2E', () => {
  ...
  beforeAll(async () => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        throw new Error(`Test file ${file} does not exist. Please ensure all test documents are present in the docs directory.`);
      }
    }
  });
  ...
  test('run processes all markdown files and generates embeddings', async () => {
    ...
    expect(files.map(f => f.filePath)).toEqual(
      expect.arrayContaining(['getting-started.md', 'api-docs.md', 'troubleshooting.md'])
    );
    ...
    const embeddingsForGettingStarted = embeddings.filter(e => e.filePath.includes('getting-started.md'));
    const embeddingsForApiDocs = embeddings.filter(e => e.filePath.includes('api-docs.md'));
    const embeddingsForTroubleshooting = embeddings.filter(e => e.filePath.includes('troubleshooting.md'));
  ...
  test('run refreshes all embeddings when shouldRefresh is true', async () => {
    ...
    const files = await prisma.file.findMany();
    const uniqueRefreshDates = new Set(files.map(f => f.latestRefresh.getTime()));
  ...
  test('run handles file deletions correctly', async () => {
    ...
    await fs.writeFile(tempFilePath, '# Temporary File\n\nThis file will be deleted.');

    // Run again to process the new file
    await run();

    // Verify the temporary file was processed
    let tempFile = await prisma.file.findUnique({ where: { filePath: 'temp-file.md' } });
    ...
    tempFile = await prisma.file.findUnique({ where: { filePath: 'temp-file.md' } });
    expect(tempFile).toBeNull();

    const deletedEmbeddings = await prisma.embedding.findMany({ where: { filePath: 'temp-file.md' } });
