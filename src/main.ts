...
async function cleanupOrphanedData(markdownFiles: MarkdownSourceType[]): Promise<void> {
...
async function validateInputs(docsRootPath: string): Promise<void> {
  try {
    ...
    throw new Error(`Invalid docs-root-path: ${docsRootPath}. Directory does not exist or is not accessible.`);
  }
}
